...
void AGun::HitScan()
{
	//Get Camera location to begin Linetracing. (Get Pawn, Get Controller, Find Location & Rotation)

	//Get/Set Pawn and Controller
	...
	//Get/Set Location and Rotation (GetPlayerViewPoint)
	FVector Start;
	FRotator Rotation;
	OwnerController->GetPlayerViewPoint(Start, Rotation);

	//End Point (Start point + VectorRotation * Distance)
	FVector End = Start + Rotation.Vector() * MaxRange;

	//LineTrace
	FHitResult Hit; 
	//Bugfix: Shooting Self
